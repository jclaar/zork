export module ZString;
import std;

using namespace std::string_view_literals;
export constexpr std::string_view  msg_string = R"~(Muddle 105 Version/Please report strange occurrences.)~";
export constexpr auto device_table = std::to_array({ R"~(A)~"sv, R"~(AI)~"sv, R"~(D)~"sv, R"~(DM)~"sv, R"~(C)~"sv, R"~(ML)~"sv, R"~(H)~"sv, R"~(AI)~"sv, R"~(L)~"sv, R"~(AI)~"sv, R"~(M)~"sv, R"~(ML)~"sv, R"~(N)~"sv, R"~(MC)~"sv, R"~(P)~"sv, R"~(ML)~"sv, R"~(U)~"sv, R"~(MC)~"sv, R"~(Z)~"sv, R"~(ML)~"sv });
export constexpr std::string_view  flushstr1 = R"~(There appears before you a threatening figure clad all over in heavy
...
export constexpr std::string_view  flushstr2 = R"~(Suddenly, a sinister, wraithlike figure appears before you, seeming
...
export constexpr std::string_view  end_herald_1 = R"~(Suddenly a sinister wraithlike figure, cloaked and hooded, appears
...
export constexpr std::string_view  end_herald_2 = R"~(
...
export constexpr std::string_view  suicidal = R"~(You clearly are a suicidal maniac.  We don't allow psychotics in the
...
export constexpr std::string_view  no_patch = R"~(What?  You don't trust me?  Why, only last week I patched a running
ITS and it survived for over 30 seconds.  Oh, well.)~";
export constexpr std::string_view  patch = R"~(Now, let me see...
Well, we weren't quite able to restore your state.  You can't have
everything.)~";
export constexpr std::string_view  death = R"~(As you take your last breath, you feel relieved of your burdens. The
...
color, even unreal.)~";
export constexpr std::string_view  life = R"~(From the distance the sound of a lone trumpet is heard.  The room
...
export constexpr std::string_view  losstr = R"~(I can't do everything, because I ran out of room.)~";
export constexpr std::string_view  backstr = R"~(He who puts his hand to the plow and looks back is not fit for the
kingdom of winners.  In any case, "back" doesn't work.)~";
export constexpr std::string_view  kitch_desc = R"~(You are in the kitchen of the white house.  A table seems to have
...
export constexpr std::string_view  lroom_desc1 = R"~(You are in the living room.  There is a door to the east.  To the
west is a cyclops-shaped hole in an old wooden door, above which is
some strange gothic lettering )~";
export constexpr std::string_view  lroom_desc2 = R"~(You are in the living room.  There is a door to the east, a wooden
door with strange gothic lettering to the west, which appears to be
nailed shut, )~";
export constexpr std::string_view  ltide_desc = R"~(It appears that the dam has been opened since the water level behind
it is low and the sluice gate has been opened.  Water is rushing
downstream through the gates.)~";
export constexpr std::string_view  htide_desc = R"~(The sluice gates on the dam are closed.  Behind the dam, there can be
seen a wide lake.  A small stream is formed by the runoff from the
lake.)~";
export constexpr std::string_view  reser_desc = R"~(You are on the reservoir.  Beaches can be seen north and south.
Upstream a small stream enters the reservoir through a narrow cleft
in the rocks.  The dam can be seen downstream.)~";
export constexpr std::string_view  dam_desc = R"~(You are standing on the top of the Flood Control Dam #3, which was
quite a tourist attraction in times far distant.  There are paths to
the north, south, east, and down.)~";
export constexpr std::string_view  cella_desc = R"~(You are in a dark and damp cellar with a narrow passageway leading
east, and a crawlway to the south.  On the west is the bottom of a
steep metal ramp which is unclimbable.)~";
export constexpr std::string_view  mirr_desc = R"~(You are in a large square room with tall ceilings.  On the south wall
is an enormous mirror which fills the entire wall.  There are exits
on the other three sides of the room.)~";
export constexpr std::string_view  trolldesc = R"~(A nasty-looking troll, brandishing a bloody axe, blocks all passages
out of the room.)~";
export constexpr std::string_view  trollout = R"~(An unconscious troll is sprawled on the floor.  All passages out of
the room are open.)~";
export constexpr std::string_view  cyclokill = R"~(The cyclops, tired of all of your games and trickery, eats you.
The cyclops says 'Mmm.  Just like mom used to make 'em.')~";
export constexpr std::string_view  cyclofood = R"~(The cyclops says 'Mmm Mmm.  I love hot peppers!  But oh, could I use
...
export constexpr std::string_view  cyclolook = R"~(A cyclops, who looks prepared to eat horses (much less mere
...
export constexpr std::string_view  cycloeye = R"~(The cyclops is standing in the corner, eyeing you closely.  I don't
think he likes you very much.  He looks extremely hungry even for a
cyclops.)~";
export constexpr std::string_view  robber_c_desc = R"~(There is a suspicious-looking individual, holding a bag, leaning
against one wall.  He is armed with a vicious-looking stiletto.)~";
export constexpr std::string_view  robber_u_desc = R"~(There is a suspicious-looking individual lying unconscious on the
ground.  His bag and stiletto seem to have vanished.)~";
export constexpr std::string_view  resdesc = R"~(However, with the water level lowered, there is merely a wide stream
running through the center of the room.)~";
export constexpr std::string_view  gladesc = R"~(This is a large room, with giant icicles hanging from the walls
and ceiling.  There are passages to the north and east.)~";
export constexpr std::string_view  glacier_win = R"~(The torch hits the glacier and explodes into a great ball of flame,
...
export constexpr std::string_view  rusty_knife_str = R"~(As the knife approaches its victim, your mind is submerged by an
...
slits your throat.)~";
export constexpr std::string_view  cursestr = R"~(A ghost appears in the room and is appalled at your having
...
export constexpr std::string_view  torch_desc = R"~(This is a large room with a prominent doorway leading to a down
...
export constexpr std::string_view  dome_desc = R"~(You are at the periphery of a large dome, which forms the ceiling
of another room below.  Protecting you from a precipitous drop is a
wooden railing which circles the dome.)~";
export constexpr std::string_view  hellgate = R"~(You are outside a large gateway, on which is inscribed 
...
export constexpr std::string_view  exor1 = R"~(The bell suddenly becomes red hot and falls to the ground. The
...
takes shape.)~";
export constexpr std::string_view  exor2 = R"~(The flames flicker wildly and appear to dance.  The earth beneath
your feet trembles, and your legs nearly buckle beneath you.
The spirits cower at your unearthly power.)~";
export constexpr std::string_view  exor3 = R"~(Each word of the prayer reverberates through the hall in a deafening
...
export constexpr std::string_view  exor4 = R"~(The tension of this ceremony is broken, and the wraiths, amused but
shaken at your clumsy attempt, resume their hideous jeering.)~";
export constexpr std::string_view  xorcst2 = R"~(There is a clap of thunder, and a voice echoes through the
cavern: "Begone, chomper!"  Apparently, the voice thinks you
are an evil spirit, and dismisses you from the realm of the living.)~";
export constexpr std::string_view  lld_desc = R"~(You have entered the Land of the Living Dead, a large desolate room.
...
export constexpr std::string_view  lld_desc1 = R"~(Amid the desolation, you spot what appears to be your head,
at the end of a long pole.)~";
export constexpr std::string_view  bro1 = R"~(The mailing label on this glossy brochure from MIT Tech reads:

		)~";
export constexpr std::string_view  bro2 = R"~(
...
export constexpr auto drownings
...
export constexpr auto cyclomad
...
export constexpr auto jumploss
...
export constexpr auto bat_drops
= std::to_array({ R"~(MINE1)~"sv, R"~(MINE2)~"sv, R"~(MINE3)~"sv, R"~(MINE4)~"sv, R"~(MINE5)~"sv, R"~(MINE6)~"sv, R"~(MINE7)~"sv, R"~(TLADD)~"sv, R"~(BLADD)~"sv });
export constexpr std::string_view  machine_desc = R"~(This is a large room which seems to be air-conditioned.  In one
...
export constexpr std::string_view  over_falls_str = R"~(I didn't think you would REALLY try to go over the falls in a
...
export constexpr std::string_view  over_falls_str1 = R"~(Unfortunately, a rubber raft doesn't provide much protection from
the unfriendly sorts of rocks and boulders one meets at the bottom of
many waterfalls.  Including this one.)~";
export constexpr auto swimyuks
...
export constexpr std::string_view  grue_desc1 = R"~(The grue is a sinister, lurking presence in the dark places of the
...
to tell the tale.)~";
export constexpr std::string_view  grue_desc2 = R"~(There is no grue here, but I'm sure there is at least one lurking
in the darkness nearby.  I wouldn't let my light go out if I were
you!)~";
export constexpr std::string_view  brick_boom = R"~(Now you've done it.  It seems that the brick has other properties
than weight, namely the ability to blow you to smithereens.)~";
export constexpr std::string_view  hook_desc = R"~(There is a small hook attached to the rock here.)~";
export constexpr std::string_view  greek_to_me = R"~(This book is written in a tongue with which I am unfamiliar.)~";
export constexpr std::string_view  gnome_desc = R"~(A volcano gnome seems to walk straight out of the wall and says
...
export constexpr std::string_view  headstr1 = R"~(Although the implementers are dead, they foresaw that some cretin
would tamper with their remains.  Therefore, they took steps to
punish such actions.)~";
export constexpr std::string_view  headstr = R"~(Unfortunately, we've run out of poles.  Therefore, in punishment for
your most grievous sin, we shall deprive you of all your valuables,
and of your life.)~";
export constexpr std::string_view  cagestr = R"~(As you reach for the sphere, a steel cage falls from the ceiling
to entrap you.  To make matters worse, poisonous gas starts coming
into the room.)~";
export constexpr std::string_view  robotdie = R"~(The robot is injured (being of shoddy construction) and falls to the
floor in a pile of garbage, which disintegrates before your eyes.)~";
export constexpr std::string_view  vapors = R"~(Just before you pass out, you notice that the vapors from the
flask's contents are fatal.)~";
export constexpr std::string_view  crushed = R"~(The room seems to have become too small to hold you.  It seems that
the walls are not as compressible as your body, which is somewhat
demolished.)~";
export constexpr std::string_view  icemung = R"~(The door to the room seems to be blocked by sticky orange rubble
from an explosion.  Probably some careless adventurer was playing
with blasting cakes.)~";
export constexpr std::string_view  iceblast = R"~(You have been blasted to smithereens (wherever they are).)~";
export constexpr std::string_view  cmach_desc = R"~(This is a large room full of assorted heavy machinery.  The room
...
export constexpr std::string_view  spindizzy = R"~(According to Prof. TAA of MIT Tech, the rapidly changing magnetic
...
export constexpr std::string_view  spinrobot = R"~(According to Prof. TAA of MIT Tech, the rapidly changing magnetic
...
export constexpr std::string_view  robot_crush = R"~(As the robot reaches for the sphere, a steel cage falls from the
...
export constexpr std::string_view  poison = R"~(Time passes...and you die from some obscure poisoning.)~";
export constexpr std::string_view  alarm_voice = R"~(A metallic voice says 'Hello, Intruder!  Your unauthorized presence
...
export constexpr std::string_view  teller_desc = R"~(You are in a small square room, which was used by a bank officer
...
On the )~";
export constexpr std::string_view  zgnome_desc = R"~(An epicene gnome of Zurich wearing a three-piece suit and carrying a
...
export constexpr std::string_view  zgnome_pop = R"~('I wouldn't put THAT in a safety deposit box,' remarks the gnome with
disdain, tossing it over his shoulder, where it disappears with an
understated 'pop'.)~";
export constexpr std::string_view  zgnome_pop_1 = R"~('Surely you jest,' he says.  He tosses the brick over his shoulder,
and disappears with an understated 'pop'.)~";
export constexpr std::string_view  zgnome_bye = R"~(The gnome looks impatient:  'I may have another customer waiting;
you'll just have to fend for yourself, I'm afraid.  He disappears,
leaving you alone in the bare room.)~";
export constexpr std::string_view  tree_desc = R"~(You are about 10 feet above the ground nestled among some large
branches.  The nearest branch above you is above your reach.)~";
export constexpr std::string_view  opera = R"~(The canary chirps, slightly off-key, an aria from a forgotten opera.
...
export constexpr std::string_view  complex_desc = R"~(....
...
export constexpr std::string_view  gigo = R"~(The item vanishes into the slot.  A moment later, a previously 
unseen sign flashes 'Garbage In, Garbage Out' and spews
the )~";
export constexpr std::string_view  confiscate = R"~(The card slides easily into the slot and vanishes and the metal door
...
export constexpr std::string_view  pal_room = R"~(This is a small and rather dreary room, which is eerily illuminated
...
export constexpr std::string_view  pal_door = R"~( side of the room is a massive wooden door, near
the top of which, in the center, is a window barred with iron.
A formidable bolt lock is set within the door frame.  A keyhole)~";
export constexpr std::string_view  slide_desc = R"~(This is a small chamber, which appears to have been part of a
...
a small opening.)~";
export constexpr std::string_view  slippery = R"~(As you descend, you realize that the rope is slippery from the grime
of the coal chute and that your grasp will not last long.)~";
export constexpr std::string_view  tomb_desc1 = R"~(This is the Tomb of the Unknown Implementer.
...
export constexpr std::string_view  tomb_desc2 = R"~( Above the entrance is the
cryptic inscription:

		     "Feel Free."
)~";
export constexpr std::string_view  crypt_desc = R"~(Though large and esthetically pleasing the marble crypt is empty; the
...
export constexpr std::string_view  pass_word_inst = R"~(Suddenly, as you wait in the dark, you begin to feel somewhat
...
export constexpr std::string_view  miropen = R"~(The mirror is mounted on a panel which has been opened outward.)~";
export constexpr std::string_view  panopen = R"~(The panel has been opened outward.)~";
export constexpr std::string_view  hallway = R"~(This is a part of the long hallway.  The east and west walls are
...
export constexpr std::string_view  guardkill = R"~(The Guardians awake, and in perfect unison, utterly destroy you with
their stone bludgeons.  Satisfied, they resume their posts.)~";
export constexpr std::string_view  guardkill1 = R"~(Suddenly the Guardians realize someone is trying to sneak by them in
...
satisfied.)~";
export constexpr std::string_view  guard_attack = R"~(Attacking the Guardians is about as useful as attacking a stone wall.
Unfortunately for you, your futile blow attracts their attention, and
they manage to dispatch you effortlessly.)~";
export constexpr std::string_view  mirbreak = R"~(The mirror breaks, revealing a wooden panel behind it.)~";
export constexpr std::string_view  mirbroke = R"~(The mirror has already been broken.)~";
export constexpr std::string_view  panelbreak = R"~(To break the panel you would have to break the mirror first.)~";
export constexpr std::string_view  panelbroke = R"~(The panel is not that easily destroyed.)~";
export constexpr std::string_view  guardstr = R"~(, identical stone statues face each other from
...
export constexpr std::string_view  inside_mirror_1 = R"~(You are inside a rectangular box of wood whose structure is rather
...
export constexpr std::string_view  mirror_pole_desc = R"~(     The long pole at the center of the bar extends from the ceiling
...
T-bar are pointing )~";
export constexpr auto longdirs
= std::to_array({ R"~(north)~"sv, R"~(northeast)~"sv, R"~(east)~"sv, R"~(southeast)~"sv, R"~(south)~"sv, R"~(southwest)~"sv, R"~(west)~"sv, R"~(northwest)~"sv });
export constexpr auto nums = std::to_array({ R"~(one)~"sv, R"~(two)~"sv, R"~(three)~"sv, R"~(four)~"sv, R"~(five)~"sv, R"~(six)~"sv, R"~(seven)~"sv, R"~(eight)~"sv });
export constexpr std::string_view  master_attack = R"~(The dungeon master is taken momentarily by surprise.  He dodges your
...
export constexpr std::string_view  inq_lose = R"~(" The dungeon master,
...
that, he departs.)~";
export constexpr std::string_view  quiz_win = R"~(The dungeon master, obviously pleased, says "You are indeed a
...
export constexpr std::string_view  quiz_rules = R"~(The knock reverberates along the hall.  For a time it seems there
...
export constexpr std::string_view  ewc_desc = R"~(This is a large east-west corridor which opens onto a northern
...
export constexpr std::string_view  parapet_desc = R"~(You are standing behind a stone retaining wall which rims a large
...
the number ')~";
export constexpr std::string_view  win_totally = R"~(     As you gleefully examine your new-found riches, the Dungeon
...
export constexpr std::string_view  deadend = R"~(Dead End)~";
export constexpr std::string_view  sdeadend = R"~(You have come to a dead end in the maze.)~";
export constexpr std::string_view  stfore = R"~(This is a forest, with trees in all directions around you.)~";
export constexpr std::string_view  forest = R"~(Forest)~";
export constexpr std::string_view  fordes = R"~(This is a dimly lit forest, with large trees all around.  To the
east, there appears to be sunlight.)~";
export constexpr std::string_view  fortree = R"~(This is a dimly lit forest, with large trees all around.  One
particularly large tree with some low branches stands here.)~";
export constexpr std::string_view  tchomp = R"~(The troll fends you off with a menacing gesture.)~";
export constexpr std::string_view  mazedesc = R"~(This is part of a maze of twisty little passages, all alike.)~";
export constexpr std::string_view  smazedesc = R"~(Maze)~";
export constexpr std::string_view  nofit = R"~(You cannot fit through this passage with that load.)~";
export constexpr std::string_view  mindesc = R"~(This is a non-descript part of a coal mine.)~";
export constexpr std::string_view  smindesc = R"~(Coal mine)~";
export constexpr std::string_view  riverdesc = R"~(Frigid River)~";
export constexpr std::string_view  narrow = R"~(The path is too narrow.)~";
export constexpr std::string_view  view_room = R"~(This is a room used by holders of safety deposit boxes to view
...
export constexpr std::string_view  bkalarm = R"~(An alarm rings briefly and an invisible force prevents your leaving.)~";
export constexpr std::string_view  timber_untied = R"~(There is a wooden timber on the ground here.)~";
export constexpr std::string_view  timber_tied = R"~(The coil of rope is tied to the wooden timber.)~";
export constexpr std::string_view  coffin_untied = R"~(The solid-gold coffin used for the burial of Ramses II is here.)~";
export constexpr std::string_view  coffin_tied = R"~(The coil of rope is tied to Ramses II's gold coffin.)~";
